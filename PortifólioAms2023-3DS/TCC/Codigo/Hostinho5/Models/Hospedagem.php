...
class Hospedagem extends Conexao {
    ...
    public function create(){
		try {
            $stmt = $this->mysqli->prepare("INSERT INTO hospedagem (`dia_hosp`, `dia_busca`, `valor_total`, ) VALUES (?,?,?);");
			$stmt->bind_param("sss",$hospedagem->dia_hosp, $hospedagem->dia_busca, $hospedagem->valor_total);
	...
	public function listarHospedagensPorTutor($tutorCod) {
		...
		try {
			$stmt = $this->mysqli->prepare("SELECT data_hosp, data_busca, valor_total FROM hospedagem WHERE cod_tutor = ?");
	...
	public function listarHospedagensPorAnfitriao($anfitriaoCod) {
		...
		try {
			$stmt = $this->mysqli->prepare("SELECT data_hosp, data_busca, valor_total FROM hospedagem WHERE cod_anf = ?");
