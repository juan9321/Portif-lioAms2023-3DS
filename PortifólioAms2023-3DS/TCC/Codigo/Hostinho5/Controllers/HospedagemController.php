...
class HospedagemController {
    ...
    public function create() {
        $this->hospedagem->setNome($_POST['Nome_Anf']);
        $this->hospedagem->setTelefone($_POST['Telefone']);
        $this->hospedagem->setEndereco($_POST['Endereco']);
        $this->hospedagem->setValorTotal($_POST['ValorTotal']);

        $result = $this->hospedagem->create($this->hospedagem);
        if ($result) {
            echo "<script>alert('Hospedagem incluída com sucesso!');document.location='../View/login-cadastro.php'</script>";
        } else {
